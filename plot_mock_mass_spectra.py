...
def define_colors(df):
  """Define ion color mapping function"""
  color_mapping = {
    "b": "blue",
    "y": "green",
    "Y": "red",
  ...
  # Convert IonSeries from string to dictionary
  df['IonSeries'] = df['IonSeries'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)  # Parse IonSeries as dictionary if it's a string

  # find unique ions in ionseries
  unique_ions = set()
  for ion_series in df['IonSeries']:
    unique_ions.update(ion_series.keys())
  
  # Add more colors for any additional ion types found
  additional_colors = ['cyan', 'magenta', 'orange', 'brown', 'pink', 'gray']
...
def create_mass_spectrum_plot(df, color_mapping, title):
    """Plot mass spectrum with ion numbers as labels and variable ion line length"""
        ...
        for ion_type, values in ion_series.items():
            if isinstance(values, list):  # For b and y ions
                ...
                for i, mz in enumerate(values, start=0):
                    ...
                    text = ax.text(mz, line_length, label, rotation=90, verticalalignment='bottom', fontsize=8, color='black')
                    text.set_path_effects([path_effects.Stroke(linewidth=3, foreground="white"), path_effects.Normal()])

                    # Add to legend if not already present
                    if ion_type not in [handle.get_label() for handle in legend_handles]:
                        legend_handles.append(plt.Line2D([0], [0], color=color, lw=2, label=ion_type))
